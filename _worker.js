...
async function DoH查询(域名, 记录类型, DoH解析服务 = "https://cloudflare-dns.com/dns-query") {
	...
	try {
		...
		const recordType = 记录类型.toUpperCase();
		const qtype = 类型映射[recordType] || 1;
		const url = new URL(DoH解析服务);
		url.searchParams.set('name', 域名);
		url.searchParams.set('type', recordType);

		// ponytail: DoH JSON already returns name/type/TTL/data; no local DNS parser.
